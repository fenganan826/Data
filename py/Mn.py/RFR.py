...
def objective(n_estimators, max_depth, min_samples_split, min_samples_leaf):
    ...
    max_depth = int(max_depth)  # 保留该参数，但不改动你的原模型（RandomForest 默认弱学习器）
...
# 贝叶斯优化参数范围
pbounds = {
    'n_estimators': (50, 128),  # 模型学习次数范围，起始为 50 次
    'max_depth': (3, 8),
...
print(f"最优 min_samples_split: {best_min_samples_split}")
print(f"最优 min_samples_leaf: {best_min_samples_leaf}")
...
for n_estimators in np.arange(50, 201, 10):  # 这里的步长可以调整
    fold_train_mse = []
    fold_val_mse = []
    fold_train_r2 = []
    fold_val_r2 = []
